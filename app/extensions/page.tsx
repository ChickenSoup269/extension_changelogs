...
import { EXTENSIONS, CATEGORIES, type Extension, type ExtCategory, type ExtStatus } from '@/lib/data'
    ...
    <section className="max-w-[1200px] mx-auto px-10 py-14">
      ...
      <div className="mb-10">
        <h1 className="font-syne font-extrabold text-4xl tracking-tight mb-2">
          Tất cả <span className="gradient-text">Extensions</span>
        </h1>
        <p style={{ color: 'var(--muted)' }}>{EXTENSIONS.length} extension từ cộng đồng developer toàn cầu</p>
      </div>
      ...
      <div className="relative mb-6">
        <input
          ...
          placeholder="Tìm kiếm extension, tag, mô tả..."
          value={search}
      ...
      <div className="flex flex-wrap gap-2 mb-5">
        {CATEGORIES.map((cat) => (
          ...
          >
            <i className={`${cat.icon} text-xs ${category === cat.id ? 'text-white' : 'text-[var(--accent)]'}`}></i>
      ...
      <div className="flex items-center gap-2 mb-8">
        <span className="text-xs tracking-wider" style={{ color: 'var(--muted)' }}>TRẠNG THÁI:</span>
        {(['all', 'stable', 'beta', 'new'] as const).map((s) => (
          ...
          >
            {s === 'all' ? 'Tất cả' : s === 'stable' ? 'Stable' : s === 'beta' ? 'Beta' : 'Mới'}
          </button>
        ))}
      ...
      {filtered.length > 0 ? (
        ...
        <div className="text-center py-20" style={{ color: 'var(--muted)' }}>
          ...
          <p className="text-lg font-syne">Không tìm thấy extension</p>
          <p className="text-sm mt-2">Thử thay đổi từ khoá hoặc bộ lọc</p>
        </div>
      )}
          ...
          >
            <div className="flex items-start gap-4 mb-6">
              ...
              >
                {selected.icon.startsWith('/') ? (
                  <img src={selected.icon} alt={selected.name} className="w-full h-full object-cover" />
                ) : (
                  <i className={selected.icon} style={{ color: 'var(--accent)' }}></i>
                )}
              </div>
              <div>
                <h2 className="font-syne font-bold text-2xl tracking-tight">{selected.name}</h2>
            ...
            <div className="flex gap-3">
              <button
                ...
                onClick={() => { alert(`Cài đặt ${selected.name}!`); setSelected(null) }}
              >
                Cài đặt ngay
              </button>
              <button
