...
export async function fetchWebstoreStats(extensionId: string): Promise<WebstoreStats | null> {
  ...
  try {
    ...
    const userMatch = html.match(/class=["'][^"']*["']>([^<]*?)\s+users<\/div>/i) || 
                      html.match(/(\d{1,3}(,\d{3})*(\+)?)\s+users/i)
    
    // Scrape rating: looking for the numeric rating value (e.g. 4.8)
    const ratingMatch = html.match(/aria-label=["']Average rating ([0-9.]+) out of 5/i) ||
